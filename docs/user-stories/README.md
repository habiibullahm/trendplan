# User stories index

Short MVP stories for TrendPlan. Each file: **Story** → **Acceptance criteria** → **Out of scope**.

| Story | File |
|-------|------|
| Auth & onboarding | [auth-onboarding.md](auth-onboarding.md) |
| Weekly planner | [weekly-planner.md](weekly-planner.md) |
| Tren & rekomendasi | [tren-rekomendasi.md](tren-rekomendasi.md) |
| Partner week share | [partner-week-share.md](partner-week-share.md) |
| Dashboard & riwayat | [dashboard-riwayat.md](dashboard-riwayat.md) |
| Akun & pengingat | [akun-reminders.md](akun-reminders.md) |
| Demo tour | [demo-tour.md](demo-tour.md) |
| AI caption assist | [ai-caption-assist.md](ai-caption-assist.md) |
<<<<<<< HEAD
| Bantu AI coachmark | [bantu-ai-coachmark.md](bantu-ai-coachmark.md) |
=======
| Admin feedback inbox | [admin-feedback.md](admin-feedback.md) |
>>>>>>> origin/main
